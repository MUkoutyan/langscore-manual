...
現在のゲームプロジェクトの再解析を行います。

ゲーム内の文章を修正したり、新規に文章を追加した場合は必ず再解析を行って下さい。

再解析後に書き出しを行うことで、最新の内容が翻訳CSVに反映されます。

.. admonition:: 注意
   :class: warning
   
   テキストの修正や削除を行っても、CSVには修正前や削除したテキストが残り続けます。

   翻訳処理には問題ありませんが、不要な場合は手動で削除して下さい。
...
* デフォルト言語の列を埋める
 ...
 - 有効にした場合、original列の内容がデフォルト言語の列にそのままコピーされます。
 - 無効にした場合、デフォルト言語の列は空で出力されます。(既に記述されている内容は、空で上書きされません。)

* 新しいテキストを末尾に追加する
 
 - 有効にしてCSVを上書きする場合、新規に追加されたとみなされた文章がCSVの末尾に追加されます。
 - 無効の場合はマップのイベント順に依存して追加されます。
