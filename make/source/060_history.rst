...
* 「デフォルト言語の列を埋める」「新しいテキストを末尾に追加する」のフラグを追加

Ver.0.8.11 2026-03-21
********************
...
* 極稀にゲームがクラッシュする不具合の、原因となりそうな箇所を削除

仕様変更
========

* 翻訳文が見つからなかった場合、行末の改行文字を削除して再検索する処理を追加
...
* プラグインパラメータが更新されない不具合を修正 `→感謝 <https://github.com/MUkoutyan/langscore-app/issues/3>`_
...
* 原文チェックを入れて言語が有効になった場合、プラグインに有効な言語として書き出されない不具合を修正 `→感謝 <https://github.com/MUkoutyan/langscore-app/issues/3>`_
* :ref:`プラグイン制作者向けの項目(MV/MZ向け) <for_plugin_creator>` のミスを修正 `→感謝 <https://github.com/MUkoutyan/langscore-app/issues/3>`_


Ver.0.8.10 2025-07-05
********************
...
* NWjs実行時で言語パッチモードが無効の場合、画像の読み込み時にクラッシュする不具合を修正。
* data/translateフォルダが存在しないと起動時の処理で無限ループとなる不具合を修正。
* プラグインのパラメータを持つScript.csvを読み込むとエラーとなる不具合を修正。
* デフォルト言語以外の言語を選択した後、デフォルト言語に戻すと翻訳文が適用されない不具合を修正。
* パッチモード使用中に、適用中の言語フォルダを削除して起動すると、言語メニューが適用中の言語のままになる不具合を修正。
* デプロイ時に必ず含める画像の項目に、正しく画像が含まれていない不具合を修正。
* 言語読み込み状態を格納する変数の項目が選択できない不具合を修正。

仕様変更
========
* サポート外の言語フォルダは読み込まないように変更。
...
不具合修正
==========
* 画像の拡張子が小文字でないと検出できない不具合を修正。
* config.jsonが無くても処理が続行され、例外が発生する不具合を修正。
* プラグイン更新時に「デプロイメントしても必ず含めるファイル」で既存のファイルパスが保持されない不具合を修正。

仕様変更
========
* MV/MZで「メモ」の項目を検出するように変更。
* Langscore.jsの配置場所をプラグインリストの先頭から末尾に変更。
* Langscore_ObserverBridge.jsプラグインを先頭に配置するように変更。
* 言語の変数パラメータの初期値を0に設定。
* 翻訳CSVのテキストエンコーディングをBom付きUTF-8に変更。
...
不具合修正
========
* 画像フォルダが多重に階層化されていると、ツリーに画像が表示されない不具合を修正
